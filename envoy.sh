...
if [ $PERF_ENABLED = 0 ]; then
    ...
    if [ $PERF_TOOL = pprof ]; then
        ...
        taskset -c $PERF_CPUSET perf record -g -o ${BASE_DIR}/perf.data -- taskset -c ${ENVOY_CPU_SET} ${ENVOY_BIN} --base-id $BASE_ID --config-path $ENVOY_CONFIG --concurrency $ENVOY_CONCURRENCY > ./envoy.log 2>&1 &
    fi
fi
